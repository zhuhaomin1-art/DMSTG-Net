...
def make_componentwise_views(model: Chapter4WindPowerModel, x: torch.Tensor, hf_noise_std: float = 0.03):
    """
    分量级增强：
...
def physics_consistency_loss(pred: torch.Tensor, max_ramp: float = 0.25):
    ...
    if pred.shape[-1] > 1:
        ...
        ramp_penalty = F.relu(ramp - max_ramp).mean()
    else:
...
def train(config):
    ...
    for epoch in range(1, config["epochs"] + 1):
        ...
        for x, y in pbar:
            ...
            # 分量级对比分支
            x1, x2 = make_componentwise_views(model, x, hf_noise_std=config["hf_noise_std"])
            ...
            loss_cl = loss_cl_global + 0.5 * (loss_cl_hf + loss_cl_trend)
