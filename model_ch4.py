...
class PeriodicEncoder(nn.Module):
    ...
    def __init__(self, in_dim: int, hidden_dim: int, num_heads: int = 4, max_len: int = 1024):
        super().__init__()
        self.proj = nn.Linear(in_dim, hidden_dim)
        self.pe = PositionalEncoding(hidden_dim, max_len=max_len)
...
class TrendEncoder(nn.Module):
    """趋势分量编码器：大核平滑 + 全局上下文门控。"""

    def __init__(self, in_dim: int, hidden_dim: int):
        super().__init__()
        self.local_smooth = nn.Sequential(
            nn.Conv2d(in_dim, hidden_dim, kernel_size=(1, 7), padding=(0, 3)),
            nn.BatchNorm2d(hidden_dim),
            nn.GELU(),
        )
...
# 论文命名别名（DMH-Net）
DMH_Net = Chapter4WindPowerModel
